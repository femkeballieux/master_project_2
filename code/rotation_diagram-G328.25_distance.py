...
lines = ['CO21','CO32']
regions= ['red_close','red_middle', 'red_far', 'blue_close', 'blue_middle', 'blue_far' ]
pixel_region_size_21 = [1.262e3, 1.258e3, 1.252e3, 1.258e3, 1.262e3, 1.253e3] #The region size in pixels
pixel_region_size_32 = [1.262e3, 1.258e3, 1.252e3, 1.258e3, 1.262e3, 1.253e3] #Important to do them seperately, as some regions get cut off
CO21_pixel_scale= (0.11 * 0.11) #arcsec^2 per pixel
CO32_pixel_scale= (0.11 * 0.11) #arcsec^2 per pixel
#Be careful, file information in carta does not update after smoothing
...
for i, region in enumerate(regions): 
    ...
    for line in lines:
        #Import data
        data=np.genfromtxt(path+'integrated_spectra_distance/'+region+'_'+line+'.txt')
        ...
        plt.savefig(path+'integrated_spectra_distance/spectrum'+ line+'_' + region +'.pdf', bbox_inches='tight')
        plt.close()

        #use trapz to integrate
        integral = np.trapz(y_data, x_data) 
        e_integral = 0.1 * integral 
        #TODO: if we end up using these, convolve with uncertainty mathilde send. "the uncertainty for an area under a curve is sqrt(N) x rms x dv; with N the number of 
        #channels on which the curve is integrated; the rms is the noise level (let me know if you did not calculated that yet) and dv is the channel width in velocity units."
    ...
    plt.text(17, ln_E_g[1]+0.05, f'N = {N:.2}' + f' $\pm$ {e_N:.1e}'+' $cm^{-2}$' ) #position is first 2 nummbers
    plt.title(region)
    plt.savefig(path+'rotation_diagrams_distance/rotation_diagram'+region+'.png', bbox_inches='tight')
    plt.close()